 ...
 */
interface SubFunction<T, K> {
    (): SubFunction<T,K>;
    (input: T[]): K[];
}

export function map<T, K>(): typeof map;
export function map<T, K>(mapper: (i: T) => K): SubFunction<T,K>;
export function map<T, K>(mapper: (i: T) => K, input: T[]): K[];

export function map<T, K>(mapper?: (i: T) => K, input?: T[]) {
    if (mapper && input) {
        return input.map(mapper);
    }
    if (mapper) {
        const subFunction = (input?: T[]) => input ? input.map(mapper) : subFunction;
        return subFunction;
    }
    return map;
